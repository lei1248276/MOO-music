...
export function getBanner(type: 0 | 1 | 2 | 3 = 2) {
  return request.get<BannerResponse>(`/banner?type=${type}`)
}

// * 获取推荐歌单
export function getRecommend(limit: number) {
  return request.get<RecommendResponse>(`/personalized?limit=${limit}`)
}

// * 获取新歌单
export function getNewSonglist(offset: number, limit: number) {
  return request.get<SonglistResponse>(`/top/playlist?limit=${limit}&order=hot&offset=${offset}`)
}
...
export function getNewAlbum(limit = 3) {
  return request.get<AlbumResponse>(`/album/list?limit=${limit}`)
}
